# -*- coding: utf-8 -*-
"""
Created on Thu Jan 19 13:38:09 2023
...
surrounding = ["Serene", "Tumultuous"]

# using list comprehension 
# to compute all possible permutations
all_combinations = [[i, j, k, l,o] for i in weather_condition
                                   for j in day_time
                                   for k in area
...
for i,j,k,l,o in all_combinations:    
    print("driving in a %s weather in the %s in the %s area %s traffic, the surrounding is %s"%(i,j,k,l,o)) 
    prompt = "driving in a %s weather in the %s in the %s area %s traffic, the surrounding is %s"%(i,j,k,l,o)
...
#generated_image_name = "generated_image.png"  # any name you like; the filetype should be .png
    ...
    produced_images_names = [f"produced_image_{i}_{j}_{k}_{l}_{o}_{m}.png" for m in range(4)]  # create names
...
# all_images = [[i, j, k, l, o, m] for i in weather_condition
#                                  for j in day_time
#                                  for k in area
#                                  for l in traffic_state
#                                  for o in surrounding
#                                  for m in range (4)
#                                  ]
# parameters = ['weather_condition', 'day_time', 'area', 'traffic_state','surrounding',  'Count']
# df = pd.DataFrame(all_images, columns=parameters)
# print(df)

# # write DataFrame to Excel file with sheet name
# df.to_excel('HFES_DALLE_Driving_Results.xlsx', sheet_name='Suggested_Parametrs', index = False) # index = False to get rid of the auto-generated numbers besides each row
